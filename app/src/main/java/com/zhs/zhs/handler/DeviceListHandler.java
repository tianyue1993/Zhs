...
public class DeviceListHandler extends BaseResponseHandler {

    public DeviceListHandler() {
        this(DEFAULT_CHARSET);
    }

    public DeviceListHandler(String encoding) {
        super(encoding);
    }
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 1) {
                                    onSuccess(JSON.parseObject(response.toString(), MyDevice.class));
                                } else if (code == 0) {
