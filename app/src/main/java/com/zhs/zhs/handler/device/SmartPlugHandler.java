...
public class SmartPlugHandler extends BaseResponseHandler {

    public SmartPlugHandler() {
        this(DEFAULT_CHARSET);
    }

    public SmartPlugHandler(String encoding) {
        super(encoding);
    }
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 1) {
                                    onSuccess(JSON.parseObject(response.toString(), SmartPlugData.class));
                                } else {
                                    onFailure();
                                }
                            } else {
