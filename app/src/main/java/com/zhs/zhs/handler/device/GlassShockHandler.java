...
public class GlassShockHandler extends BaseResponseHandler {

    public GlassShockHandler() {
        this(DEFAULT_CHARSET);
    }

    public GlassShockHandler(String encoding) {
        super(encoding);
    }
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 1) {
                                    onSuccess(JSON.parseObject(response.toString(), GlassShockData.class));
                                } else {
                                    onFailure();
                                }
                            } else {
