...
public class ConcentratorHandler extends BaseResponseHandler {

    public ConcentratorHandler() {
        this(DEFAULT_CHARSET);
    }

    public ConcentratorHandler(String encoding) {
        super(encoding);
    }
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 1) {
                                    onSuccess(JSON.parseObject(response.toString(), CnncentratorData.class));
                                } else {
                                    onFailure();
                                }
                            } else {
