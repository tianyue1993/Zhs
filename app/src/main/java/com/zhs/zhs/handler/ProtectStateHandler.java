...
import com.zhs.zhs.entity.FamilyState;
...
public class ProtectStateHandler extends BaseResponseHandler {

    public ProtectStateHandler() {
        this(DEFAULT_CHARSET);
    }

    public ProtectStateHandler(String encoding) {
        super(encoding);
    }

    public void onSuccess(FamilyState state) {
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 1) {
                                    onSuccess(JSON.parseObject(response.toString(), FamilyState.class));
                                } else if (code == 0) {
