...
public class DefaultFamilyHandler extends BaseResponseHandler {

    public DefaultFamilyHandler() {
        this(DEFAULT_CHARSET);
    }

    public DefaultFamilyHandler(String encoding) {
        super(encoding);
    }

    public void onSuccess(DefaultFamily family) {
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 1) {
                                    onSuccess(JSON.parseObject(response.toString(), DefaultFamily.class));
                                } else if (code == 0) {
