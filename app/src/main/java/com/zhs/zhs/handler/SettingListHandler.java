...
public class SettingListHandler extends BaseResponseHandler {

    public SettingListHandler() {
        this(DEFAULT_CHARSET);
    }

    public SettingListHandler(String encoding) {
        super(encoding);
    }
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 1) {
                                    onSuccess(JSON.parseObject(response.toString(), SettingList.class));
                                } else if (code == 0) {
