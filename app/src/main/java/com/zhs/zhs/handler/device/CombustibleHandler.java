...
public class CombustibleHandler extends BaseResponseHandler {

    public CombustibleHandler() {
        this(DEFAULT_CHARSET);
    }

    public CombustibleHandler(String encoding) {
        super(encoding);
    }
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 1) {
                                    onSuccess(JSON.parseObject(response.toString(), CombustibleGasData.class));
                                } else {
                                    onFailure();
                                }
                            } else {
